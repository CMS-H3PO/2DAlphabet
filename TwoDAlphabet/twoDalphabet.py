...
# -------- STAT METHODS ------------------ #
    ...
    def Limit(self, subtag, card_or_w='card.txt', blindData=True, verbosity=0, strategy=0,
                    setParams={}, condor=False, eosRootfiles=None, extra=''):
        if subtag == '': 
...
def _runMLfit(cardOrW, blinding, verbosity, strategy, rMin, rMax, setParams, usePreviousFit=False, extra=''):
    ...
    params_to_set = ','.join(['mask_%s_SIG=1'%r for r in blinding]+['%s=%s'%(p,v) for p,v in setParams.items()]+['r=0'])
...
def _runLimit(blindData, verbosity, strategy, setParams, card_or_w='card.txt', condor=False, extra=''):
    ...
    limit_cmd = 'combine -M AsymptoticLimits -d {card_or_w} --saveWorkspace --cminDefaultMinimizerStrategy {strategy} {param_opt} {blind_opt} -v {verb} {extra}' 
