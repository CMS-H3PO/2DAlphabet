...
def generate(year, ttbar_path, data_path, region):
    from plotRpf_VR import x_min, x_max, y_min, y_max, rpf_boosted_VR
    import ROOT
    ...
    # histograms: boosted
    mj_vs_mjj_fail_b_ttbar = ttbar_file.Get('mj_vs_mjj_{0}_fail_boosted_nominal'.format(region))
    mj_vs_mjj_pass_b_ttbar = ttbar_file.Get('mj_vs_mjj_{0}_pass_boosted_nominal'.format(region))

    mj_vs_mjj_fail_b_data = data_file.Get('mj_vs_mjj_{0}_fail_boosted_nominal'.format(region))
    mj_vs_mjj_pass_b_data = data_file.Get('mj_vs_mjj_{0}_pass_boosted_nominal'.format(region))



    for i in range(1,mj_vs_mjj_pass_b_data.GetNbinsX() + 1):
        for j in range(1,mj_vs_mjj_pass_b_data.GetNbinsY() + 1):
            
            x_center = mj_vs_mjj_pass_b_data.GetXaxis().GetBinCenter(i)
            y_center = mj_vs_mjj_pass_b_data.GetYaxis().GetBinCenter(j)
            
            if x_center < x_min or x_center > x_max: continue
            if y_center < y_min or y_center > y_max: continue
            
            # boosted
            data_fail_b = mj_vs_mjj_fail_b_data.GetBinContent(i,j)
            ttbar_fail_b = mj_vs_mjj_fail_b_ttbar.GetBinContent(i,j)
            ttbar_pass_b = mj_vs_mjj_pass_b_ttbar.GetBinContent(i,j)
            
            qcd_fail_b = (data_fail_b - ttbar_fail_b)
            
            if qcd_fail_b < 0.:
                print('WARNING: qcd_fail_b negative for (mjj, mj)=({0}, {1}). Manually set to 0 (data_fail_b={2}, ttbar_fail_b={3})'.format(x_center, y_center, data_fail_b, ttbar_fail_b))
                qcd_fail_b = 0.
            ...
            if qcd_pass_b_exp < 0.:
                print('WARNING: Boosted transfer function negative for (mjj, mj)=({0}, {1})'.format(x_center, y_center))
                qcd_pass_b_exp = 0.
            ...
            mj_vs_mjj_pass_b_data.SetBinContent(i,j, data_pass_b)
            

            # write histograms
            data_file.cd()
            mj_vs_mjj_pass_b_data.Write("", ROOT.TObject.kOverwrite)
